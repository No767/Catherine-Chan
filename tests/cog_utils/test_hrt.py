...
def test_calc_e_poml():
    ...
    assert second_res["pmol_l_output"] == 7.3424674 and second_res["pg_ml_output"] == 0


def test_calc_prog():
    res = calc_prog_or_t(AMOUNT, PROG_CONSTANT, NMOL_L)
    assert res["nmol_l_output"] == 0.6289308176100629 and res["ng_dl_output"] == 0

    second_res = calc_prog_or_t(AMOUNT, PROG_CONSTANT, NG_DL)
    assert second_res["ng_dl_output"] == 6.36 and second_res["nmol_l_output"] == 0


def test_calc_t():
    res = calc_prog_or_t(AMOUNT, T_CONSTANT, NMOL_L)
    assert res["nmol_l_output"] == 57.68626172833809 and res["ng_dl_output"] == 0

    second_res = calc_prog_or_t(AMOUNT, T_CONSTANT, NG_DL)
    assert second_res["ng_dl_output"] == 0.0693406 and second_res["nmol_l_output"] == 0
