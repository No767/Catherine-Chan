...
from utils.pages import CatherinePages
...
class TermSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: TermsPages, page: dict[str, Any]) -> Embed:
        menu.embed.clear_fields()
        entry = self.format_info(page)

        menu.embed.title = entry.term
        menu.embed.set_thumbnail(url=self.cog.determine_image_url(page))
...
class NounSource(menus.ListPageSource):
    ...
    def format_info(self, page: dict[str, Any]) -> NounInfo:
        ...
        return NounInfo(
            masc="\n".join(
                map(_fmt_prefix, f"{page['masc']}|{page['mascPl']}".split("|"))
            ),
            fem="\n".join(
                map(_fmt_prefix, f"{page['fem']}|{page['femPl']}".split("|"))
            ),
            neutral="\n".join(
                map(_fmt_prefix, f"{page['neutr']}|{page['neutrPl']}".split("|"))
            ),
            author=self.determine_author(page["author"]),
        )
...
class InclusiveSource(menus.ListPageSource):
    ...
    def format_info(self, entries: dict[str, Any]) -> InclusiveInfo:
        return InclusiveInfo(
            instead_of="\n".join(
                f"- ~~{value}~~" for value in entries["insteadOf"].split("|")
            ),
            say="\n".join(f"- **{value}**" for value in entries["say"].split("|")),
            because=entries["because"],
            clarification=entries["clarification"],
            author=self.cog.determine_author(entries["author"]),
        )

    async def format_page(self, menu: InclusivePages, page: dict[str, Any]) -> Embed:
        menu.embed.clear_fields()
        entry = self.format_info(page)
...
class Dictionary(commands.GroupCog, name="dictionary"):
    ...
    def determine_image_url(self, entry: dict[str, Any]) -> str:
        ...
        if entry["images"] and "[object Object]" not in entry["images"]:
    ...
    def format_inline_term_reference(self, content: str, entities: list[str]):
        ...
        fmt_regex = re.compile(
            r"({})".format("|".join(map(re.escape, replacements.keys())))
        )
