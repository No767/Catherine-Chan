...
from utils.embeds import Embed, FullErrorEmbed
from utils.modal import CatherineModal
        ...
        async with self.pool.acquire() as connection:
            ...
            try:
                ...
                await interaction.response.send_message(
                    f"Suggestion entirely failed for unknown reason ({error!s})",
                    ephemeral=True,
                )
            else:
                await tr.commit()
...
class ProfilePageSource(menus.PageSource):
    ...
    # Pyright is incorrect here, the actual docstring says Optional[int]
    def get_max_pages(self) -> Optional[int]:  # pyright: ignore[reportIncompatibleMethodOverride]
        return 2
...
class Pronouns(commands.GroupCog, name="pronouns"):
    ...
    def parse_pronouns_sentence(
        ...
        regex = re.compile(
            r"({}[s]?)".format("|".join(map(re.escape, replacements.keys())))
        )
    ...
    def validate(self, sentence: str) -> bool:
        ...
        return all(var in valid_variables for var in result)
