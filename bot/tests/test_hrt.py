...
def test_calc_estradiol(cog: HRTConversion):
    ...
    assert isclose(pm_convert["pmol_l"], AMOUNT) and isclose(pm_convert["pg_ml"], 54.47759972349349)
    assert isclose(pg_convert["pmol_l"], 734.24674) and isclose(pg_convert["pg_ml"], AMOUNT)
...
def test_calc_testosterone(cog: HRTConversion):
    ...
    assert isclose(nm_convert["nmol_l"], 200) and isclose(nm_convert["ng_dl"], 5768.626172833809)
    assert isclose(ng_convert["nmol_l"], 6.934060000000001) and isclose(ng_convert["ng_dl"], 200)
...
def test_calc_progesterone(cog: HRTConversion):
    ...
    assert isclose(nm_convert["nmol_l"], 200) and isclose(nm_convert["ng_ml"], 62.893081761006286)
...
def test_calc_prolactin(cog: HRTConversion):
    ...
    assert isclose(ng_convert["miu_l"], 4255.31914893618) and isclose(ng_convert["ng_ml"], AMOUNT)
